...
function getFractionalPart(num) {
  ...
  if (typeof num !== "number") {
    ...
    const round = Math.trunc(num);
    const diff = num - round;
    const fractional = Math.fround(diff);
...
function compareAndRound(num1, num2) {
  ...
  if (typeof num1 !== "number" || typeof num2 !== "number") {
    ...
    const roundMaxNum = Math.round(maxNum);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  const convTotalPrice = Math.fround(totalPrice);
  const convMaxPrice = Math.fround(maxPrice);

  return convTotalPrice > convMaxPrice;
}
